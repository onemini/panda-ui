<script setup lang="ts" name="pd-menu-item">
...
const computedClses = computed(() => {
  return [
    'pd-menu--item',
    {
      'pd-menu--item__active': props.current.path && props.current.path === router?.currentRoute?.value.path,
    },
  ]
})
</script>
<template>
  <div :class="computedClses" :style="computedStyle" @click="onClickMenuItem">
    <div class="pd-menu--item__wrap">
      <span class="menu-icon" v-if="props.current">
        <pd-icon :size="14" :name="props.current.icon" />
      </span>
      <slot></slot>
    </div>
  </div>
