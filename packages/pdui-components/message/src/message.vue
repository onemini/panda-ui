...
<template>
  <transition name="pd-msg-fade" @before-leave="$emit('close')" @after-leave="$emit('destroy')">
    <div class="pd-message" v-show="visible" :style="computedStyle">
      <pd-icon v-if="props.type" :name="messageIconNameList[props.type]" :color="computedIconColor" />
      {{ message }}
    </div>
  </transition>
</template>
