  ...
  setup(props, { slots, expose }) {
    ...
    const computedCls = computed(() => {
      return [
        `pd-tab`,
        {
          'pd-tab--segmented': props.type === 'segmented',
        },
      ]
    })
    return () => {
      ...
      const Content = (
        <div class={computedCls.value}>
          <div class="pd-tab--header" ref={headerRef}>
            <span class="header--active__block" ref={blockRef}></span>
            {renderHeader()}
          </div>
          {slots.default() && slots.default().children ? <div class="pd-tab--content">{slots.default()}</div> : null}
        </div>
      )
      return Content
    }
